...
function simplecatch_filter_wp_title( $title ) {
	...
	if ( $paged >= 2 || $page >= 2 ) {
		$filtered_title .= ' &#124; ' . sprintf( 'صفحه %s', max( $paged, $page ) );
	}
...
function simplecatch_continue_reading() {
	...
	return ' <a class="readmore" Title ="'. esc_attr( the_title_attribute( 'echo=0' ) ) .'" href="'. esc_url( get_permalink() ) . '">' . sprintf( __( '%s', 'simplecatch' ), esc_attr( $more_tag_text ) ) . '</a>';
}
	...
	if( ( !$simplecatch_favicon = get_transient( 'simplecatch_favicon' ) ) ) {
		...
		if ( $options[ 'remove_favicon' ] == "0" ) :
			...
			if ( !empty( $options[ 'fav_icon' ] ) ) :
				$simplecatch_favicon = '<link type="image/x-icon" href="'.esc_url( $options[ 'fav_icon' ] ).' rel="shortcut icon" />'; 	
			else:
				// if empty fav_icon on theme options, display default fav icon
				$simplecatch_favicon = '<link type="image/x-icon" href="'. get_template_directory_uri() .'/images/favicon.ico" rel="shortcut icon" />';
			endif;
		endif;
					...
					<div class="featured">
						<div class="slide-image">';
							if( has_post_thumbnail() ) {
								...
								$simplecatch_sliders .= '<a href="' . get_permalink() . '" title="پیوند به '.the_title('','',false).'">';
		...
		<div <?php post_class(); ?>>
            ...
            <ul class="post-by">
                <li class="no-padding-left"><a href="<?php echo get_author_posts_url(get_the_author_meta( 'ID' )); ?>" 
                    title="<?php echo esc_attr( get_the_author_meta( 'display_name' ) ); ?>"><?php the_author_meta( 'display_name' ); ?></a></li>
                <li><?php the_time( 'j F Y' ); ?></li>
                <li><?php comments_popup_link( 'بدون ديدگاه', '1 ديدگاه', '% ديدگاه' ); ?></li>
				<li><a href = "<?php bloginfo('url'); echo '/?print='; the_ID(); ?>">چاپ</a></li>
            </ul>
            <?php the_content();
			...
			$tag = get_the_tags();
			if (! $tag ) { ?>
			<div class='print'><a href = "<?php bloginfo('url'); echo '/?print='; the_ID(); ?>">چاپ</a> | کد خبر: <?php the_ID(); ?></div>
				<div class='tags'>موضوعات: <?php the_category(', '); ?> </div>
				
			<?php } else { ?>
			
			<div class='print'><a href = "<?php bloginfo('url'); echo '/?print='; the_ID(); ?>">چاپ</a> | کد خبر: <?php the_ID(); ?></div>
			<?php
					 the_tags( '<div class="tags"> کلیدواژه ها: ', ', ', '</div>'); 
			} ?>
		</div> <!-- .post -->
	<?php endif;
}
...
function simplecatch_display_div() {
	echo '<div id="main" class="layout-978"> '; if ( function_exists( 'dynamic_sidebar' ) ) { dynamic_sidebar( 'JSTicker' ); }
	$options = get_option( 'simplecatch_options' );
	if( empty( $options['sidebar_layout'] ) )
		$themeoption_layout='right-sidebar';
	else
